...
def compute_experience_score(candidate_exp: float, required_exp: float) -> float:
    ...
    """
    if required_exp <= 0: return 1.0 # Not a hard requirement
    
    ratio = candidate_exp / required_exp
    
    if ratio >= 1.5:
        return 1.1 # Slight bonus for over-experience, capped
    elif ratio >= 1.0:
        return 1.0 # Meets requirement perfectly
    else:
        # Partial credit gracefully scaling down
        return ratio
...
def evaluate_candidate(
    ...
    # Avg rerank score of the top evidence
    avg_rerank = 0.0
    raw_avg = 0.0
    if evidence_chunks:
        scores = [c.get("rerank_score", 0) for c in evidence_chunks]
        # Filter out NaN/Inf values from cross-encoder
        ...
        if valid_scores:
            ...
            avg_rerank = (clamped + 10.0) / 20.0  # maps -10→0.0, 0→0.5, +10→1.0
        else:
            raw_avg = 0.0
            avg_rerank = 0.5  # Neutral default if all scores are invalid
    ...
    # Base: Semantic (30%), Skills (50%), Experience (20%)
    ...
    overall_score = min(overall_score, 1.0) # ceiling
    
    # User requested skipping the generative LLM summarization step. 
    # Just return the algorithmic score directly.
